...
import { REPORT_STATUSES } from "@/lib/statuses";
import { STATUS_PIN } from "@/lib/leaflet";
...
export default async function AdminDashboard() {
  ...
  const appName = config?.app_name || "SynCity";
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-1.5">
          ...
          <p className="max-w-xl text-sm text-muted-foreground">
            {config?.tagline ||
              `${appName} — triage verified reports, track wards, and resolve civic issues.`}
          </p>
        </div>
        {kpis ? (
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {kpiCards.map((kpi) => (
          <Card key={kpi.title} className="shadow-sm transition-shadow hover:shadow-md">
              ...
              {REPORT_STATUSES.map((label) => (
                ...
                >
                  <span
                    ...
                    style={{ backgroundColor: STATUS_PIN[label] || "#64748b" }}
                  />
                  {label}
                </span>
              ))}
