...
// Web capture: there's no Vision Camera on the browser. We use a hidden file input
// with `capture="environment"` so phones open the rear camera, and desktops fall back
// to a file chooser. Geolocation comes from the browser's navigator.geolocation API.
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.center}>
        ...
        {/* Hidden camera-only file input. `capture` forces the device camera
            so users can't pick an existing image from their library. */}
        {/* @ts-ignore */}
        <input
          ref={cameraInputRef}
          type="file"
...
const styles = StyleSheet.create({
  ...
  primaryBtn: {
    ...
    minWidth: 220, height: 52, paddingHorizontal: Spacing.lg,
  ...
  warn: {
    color: '#FCA5A5', fontSize: FontSize.sm, marginTop: Spacing.md, textAlign: 'center',
  },
});
